...
const ColorSelector = ({colors, setColors, colorPicker, setColorPicker, anchors, setAnchors, label}) =>{
    ...
    return (
        <Box sx={{marginLeft:'auto', display:'flex', alignItems: 'center'}}>
            <Square sx={{backgroundColor: colors[label], '&:hover':{backgroundColor: colors[label]}}} onClick={handleClick}></Square>
            <Popover 
            ...
            <ColorTextField sx={{paddingLeft:"10px"}} onChange={handleTextColor} variant="standard" value={colors[label].substring(1)} error={!(/^[0-9A-F]{6}$/i.test(colors[label].substring(1)))}></ColorTextField>
        </Box>
    )
}
